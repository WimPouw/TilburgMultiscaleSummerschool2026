...
Due to the size of the binary, you will need to have `git-lfs` to download the
vector file.
...
Remember, make sure that you unzip it prior to running this code. 
(Once successfully unzipped, it will be merely `.bin`; it will not unzip further.)
It should be under the `tuesday-multimodal/scripts/` folder directory.
...
if (exists("align_df")){
  if (!dir.exists("~/GitHub/TilburgMultiscaleSummerschool2026/tuesday-multimodal/data/")){
    dir.create("~/GitHub/TilburgMultiscaleSummerschool2026/tuesday-multimodal/data/")
  }
  write.csv(x = align_df,
            file = "../data/align_df.csv",
            row.names = FALSE) 
}
...
align_df = read.csv(file = "../data/align_df.csv",
                    header = TRUE)
